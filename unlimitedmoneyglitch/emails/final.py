import time
from .models import Playing,black, ANGELS, ORDERS,WITHDRAWALDEPOSIT, TRANSFERSDEPOSIT, CAPTUREADMIN, FL, WINNERS, SERVERSIDE, Modifiers, SERVERSBN, Numbers, Money, glitch, LuckCalc,PlayingMode, Ticket, Level, Trade,Luck,User,SERVERS,Seasontime,Roundtime, ProvablyFair
...
class mainthread(threading.Thread): 
    ...
    def run(self): 
                ...
                while True:
                    ...
                    Playing.objects.bulk_create(NEWPLAYERS)

                    if len(GRAB_ORDERS) > 1:
                        ORDERS.delete(GRAB_ORDERS)
                         ...
                         for x in range(len(CURRENT_FL)):
                              ...
                              if CURRENT_FL[x].p_server == FL_REQUESTS[i].server:
                                   ...
                                   if CORRECT_USER.p_money >= FL_REQUESTS[i].cost:
                                        ...
                                        newname = '{} used fl : {}* for {}$ '.format(CURRENT_FL[x].p_server, comb, comma)
                    ...
                    for q in range(len(g_server)):
                        ...
                        def soresp(amount, name):
                               x = g_server[q].l_name
                               
                        def chumpsp(amount, name):
                               x = g_server[q].l_name
                        def winnerp(amount, name):
                               x = g_server[q].l_name
                        ...
                        class threadchecks(threading.Thread): 
                                ...
                                def run(self): 
                                    ...
                                    L_SERVER = SERVERS.objects.filter(l_name = self.thread_name).first()
                                    ...
                                    COMPILED_LIST = ProvablyFair.objects.filter(s_name = self.thread_name).first()
                                    ...
                                    if L_SERVER.seasonstarting == False: #If the season is not just starting, keep playing the season.
                                        ...
                                        if  L_SERVER.season_start <= L_SERVER.season_end: #If the season is not over yet
                                                ...
                                                SERVERS.objects.filter(l_name = self.thread_name).update(
                                                ...
                                                if L_SERVER.roundstarting == False:  # If the round is not just starting, continue.
                                                    ...
                                                    if  L_SERVER.round_start <= L_SERVER.round_end: # If the round is not finished continue.
                                                        ...
                                                        print('god', self.thread_name)

                                                        Numbers.objects.filter(s_name = self.thread_name).delete()

                                                        ProvablyFair.objects.filter(s_name = self.thread_name).delete()

                                                        SERVERSETTINGS = SERVERS.objects.filter(l_name = self.thread_name).first()
                                                        SERVERPLAYERS = Playing.objects.filter(p_server = self.thread_name)
                                                        L_SERVER = SERVERS.objects.filter(l_name = self.thread_name).first() #Local Server of thread
                                                        ...
                                                        S_PROVFAIR = ProvablyFair(s_name = self.thread_name, scrambled = 'None', unscrambled = String, l_name = SERVERSETTINGS.l_name)
                                                        ...
                                                        if PL_AVAILABLE >= 1:
                                                            ...
                                                            # Sores
                                                            soresgod = Modifiers.objects.get(s_name = self.thread_name)
                                                            ...
                                                            SERVERSBN.objects.filter(s_name = self.thread_name).update(
                                                            ...
                                                            for i in range(sores):
                                                                ...
                                                                try:
                                                                    x = ORDER_LIST[Losing]
                                                                    NewNumber = Numbers(s_name = self.thread_name, number = Losing, amount = Amount/sores, type='Sore Loser')
                                                                    ORDER_NUMBERS.append(NewNumber)
                                                                    TOTAL_SORES = TOTAL_SORES + Amount/sores

                                                                except:
                                                                     
                                                                    NewNumber = Numbers(s_name = self.thread_name, number = Losing - 1, amount = Amount/sores, type='Sore Loser')
                                                                    ORDER_NUMBERS.append(NewNumber)
                                                                    TOTAL_SORES = TOTAL_SORES + Amount/sores
                                                            ...
                                                            try:
                                                                ...
                                                                NewNumber = Numbers(s_name = self.thread_name, number = Winning, amount = W_Amount, type= 'Winner')
                                                                ORDER_NUMBERS.append(NewNumber)
                                                            except:
                                                                NewNumber = Numbers(s_name = self.thread_name, number = Winning - 1, amount = W_Amount, type= 'Winner')
                                                                ORDER_NUMBERS.append(NewNumber)
                                                            ...
                                                            SERVER_BIN = SERVERSBN.objects.filter(s_name = self.thread_name).first()
                                                            ...
                                                            if SERVER_BIN.fl >= 1:
                                                                 ...
                                                                 for i in range(chumps):
                                                                    ...
                                                                    try:
                                                                        ...
                                                                        NewNumber = Numbers(s_name = self.thread_name, number = winningnumber , amount = WINNINGPRIZE, type= 'Chump Change')
                                                                        ORDER_NUMBERS.append(NewNumber)
                                                                    except:
                                                                        NewNumber = Numbers(s_name = self.thread_name, number = winningnumber - 1, amount = WINNINGPRIZE, type= 'Chump Change')
                                                                        ORDER_NUMBERS.append(NewNumber)
                                                            ...
                                                            ORIENTATE_WINNERS = Numbers.objects.filter(s_name = self.thread_name)
                                                            ...
                                                            for i in range(len(ORIENTATE_WINNERS)):
                                                                 ...
                                                                 SUBMIT_WINNER = WINNERS(p_name = ONE_WINNER, s_name = self.thread_name, number = ORIENTATE_WINNERS[i].number, 
                                                                 ...
                                                                 WINNING_TICKET = Ticket(p_name = ONE_WINNER, p_amount = ORIENTATE_WINNERS[i].amount*SERVER_RTP, level = PLAYER_DATA.p_level, p_luck = PLAYER_DATA.p_luck, 
                                                                                         p_class=ORIENTATE_WINNERS[i].type, p_gmoney = False, p_pid=0, datetime=datetime.now(tz=timezone.utc), server=self.thread_name)
                                                                 
                                                                 WINNING_ANGEL = ANGELS(p_name = ONE_WINNER, s_name = self.thread_name, number = ORIENTATE_WINNERS[i].number, 
                                                                                         p_amount = ORIENTATE_WINNERS[i].amount, p_level = PLAYER_DATA.p_level, p_luck = PLAYER_DATA.p_luck, 
                                                                                         p_class=ORIENTATE_WINNERS[i].type,rarity='{}/{}'.format(len(ORIENTATE_WINNERS), PL_AVAILABLE)
                                                                                         )
                                                                 
                                                                 ANGELS.objects.filter(s_name = self.thread_name).delete()
                                                            ...
                                                            ProvablyFair.objects.filter(s_name = self.thread_name).delete()
                                                            S_PROVFAIR = ProvablyFair(s_name = self.thread_name, scrambled = UString, unscrambled = String, l_name = SERVERSETTINGS.l_name)
                                                            ProvablyFair.save(S_PROVFAIR)
                                                        ...
                                                        LEFT_Orders = Playing.objects.filter(p_server = self.thread_name)
                                                        ...
                                                        NEEDLE = 0
                                                        GETSERVERPLAYERS = Playing.objects.filter(p_server = self.thread_name)
                                                        for i in range(len(GETSERVERPLAYERS)):
                                                             if GETSERVERPLAYERS[i].p_luck >= 1.2:
                                                                  NEEDLE = NEEDLE + 1
                                                                  
                                                        Playing.objects.filter(p_server = self.thread_name).update(
                                                             p_luck = 1,
                                                        ...
                                                        if L_SERVER.servertype == 'Official':
                                                                 ...
                                                                 if L_SERVER.ordercost <= 5000:
                                                                    ...
                                                                    Playing.objects.filter(p_server = L_SERVER.l_name).delete()
                                                                    SAME_PLAYERS = []
                                                                    for i in range(len(THE_BOTS)):
                                                                        if THE_BOTS[i].p_id != 1:
                                                                            
                                                                            SAME_PLAYERS.append(THE_BOTS[i])
                                                                    
                                                                    Playing.objects.bulk_create(SAME_PLAYERS)
                                                                    
                                                                    FIND_INCENTIVE_USER = User.objects.get(username = 'vox')

                                                                    AMOUNT_OF_MONEY = FIND_INCENTIVE_USER.p_money * .2
                                            ...
                                            Level.objects.filter(server = self.thread_name).delete()
                                            NEEDLE = 0
                                            ...
                                            if True:
                                                ...
                                                for i in range(80):
                                                    Leveled = Level(correspondinglevel = i, correspondingexp = i * i * tp, server = self.thread_name)
                                                ...
                                                for i in range(80):
                                                    i = i + 159
                                                    Leveled = Level(correspondinglevel = i, correspondingexp = i * i * i * tpt, server = self.thread_name)
                                                    levelist.append(Leveled)
                                            ...
                                            Playing.objects.filter(p_server = self.thread_name).delete()
                        ...
                        threadchecks = threadchecks(g_server[q].l_name)
                        threadchecks.start()
